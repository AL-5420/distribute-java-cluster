...
class AIOpsAgent:
    def __init__(self):
        ...
        if config.LLM_CONFIG["use_mock"]:
            ...
            print(f"Mocking a 'warning' at check >= {w_interval} and 'critical' at check >= {c_interval}.")
        else:
        ...
        for report in health_reports:
            ...
            if severity == "critical":
                ...
                if node_id in self.greylisted_nodes:
                    print(f"      -> Node '{node_id}' has recovered and is now removed from the greylist.")
    ...
    def _handle_warning_node(self, report, reason):
        ...
        if node_id not in self.greylisted_nodes:
            ...
            print(f"   -> Node '{node_id}' is not in greylist. Adding it and expanding cluster.")
            ...
            print(f"   -> Node '{node_id}' is already in greylist. Sending follow-up alert.")
            healer.send_alert(
                config.ALERTING_CONFIG["recipient_email"], node_id, reason, "warning", is_follow_up=True
            )
